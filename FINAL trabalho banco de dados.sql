...
INDEX cpf_gerente_INDEX (cpf_gerente ASC),
...
UNIQUE INDEX cpf_numero (nome, cpf ASC),
...
SELECT * FROM FUNCIONARIO WHERE salario >= 1000 AND salario <= 3000 order by salario;
...
JOIN empresa.departamento using (numero)
JOIN empresa.funcionario using (cpf);

SELECT funcionario.nome, sobrenome, salario , departamento.nome
FROM empresa.trabalha
JOIN empresa.funcionario
ON funcionario.cpf = trabalha.cpf
JOIN empresa.departamento
ON departamento.numero = trabalha.numero;
...
SELECT funcionario.nome, funcionario.sobrenome, dependentes.nome as Dependentes_Nome, dependentes.sobrenome as Dependentes_Sobrenome
FROM empresa.funcionario LEFT OUTER JOIN empresa.dependentes 
ON funcionario.cpf = dependentes.cpf_funcionario
     order by funcionario.nome;
     
-- h) Liste o nome, sobrenome, salário, nome do departamento e nome do gerente de cada funcionário;
-- criando uma subtabela 
SELECT f.nome, f.sobrenome, f.salario , d.nome as Departamento, g.nome as Gerente
FROM empresa.funcionario f,
	 empresa.departamento d,
     empresa.trabalha t,
     empresa.funcionario g
WHERE f.cpf = t.cpf
